...
T_short_parts = np.vstack((
       (np.hstack((
                    ...
                    T_back[:,T_back.shape[1]-max(T_short.shape[1]-T_front.shape[1],0):]))).reshape(1,T_short.shape[0],
                                                                               T_short.shape[1]),
       (np.hstack((
                    T_front[:,:max(T_short.shape[1]-T_back.shape[1],0)],
                    T_back[:,T_back.shape[1]-min(T_short.shape[1],T_back.shape[1]):]))).reshape(1,T_short.shape[0],
                                                                               T_short.shape[1])))
...
np.save('aar_parts_like_roc061212',like_roc[1])

#
# Parts did not do much better time to do some Waliji magic
#
