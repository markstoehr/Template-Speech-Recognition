...
stored_bg_obj = template_exp.AverageBackground()
...
for phn_class in classes_to_phns.keys():
    ...
    class_examples_bg = np.empty((num_phn_examples[phn_class],
                                  stored_bg_obj.E.shape[0]),
                                 dtype=np.float32)
    class_examples_lengths = np.lengths(num_phn_examples[phn_class],
                                        dtype = np.uint16)
    cur_example = 0
    for datum in xrange(num_data):
        phns = np.load(data_path + str(datum+1)+'phns.npy')
        ...
        for phn_id,phn in enumerate(phns):
            datum_phn_class = model_class[phn]
            ...
            class_examples5[cur_example] = np.hstack(
                (E5[:,datum_phn_loc:min(
                 ...
                 np.tile(E[5,-1],(datum_phn_loc+max_phn_lengths[phn_class]
                                  - E5.shape[1],
                                  1)).T))
            class_examples3[cur_example] = np.hstack(
                (E3[:,datum_phn_loc:min(
                            E3.shape[1],
                            datum_phn_loc + max_phn_lengths[phn_class])],
                 np.tile(E[3,-1],(datum_phn_loc+max_phn_lengths[phn_class]
                                  - E3.shape[1],
                                  1)).T))
            ...
            if datum_phn_loc > 20:
                ...
                class_examples_bg[cur_exampled] = stored_bg_obj.E[:]
            if phn_id >0:
                example_context[cur_example][0] = phns[phn_id-1]
            else:
                example_context[cur_example][0] = -1
            if phn_id < phns.shape[0]:
                example_context[cur_example][1] = phns[phn_id+1]
            else:
    ...
    np.save(data_dir+'class_examples3',class_examples3)
    np.save(data_dir+'class_examples5',class_examples5)
    np.save(data_dir+'class_examples_lengths',class_examples_lengths)
    np.save(data_dir+'class_examples_bgs',class_examples_bg)
    np.save(data_dir+'example_utt_id_E_loc',example_utt_id_E_loc)
    np.save(data_dir+'example_context',example_context)
