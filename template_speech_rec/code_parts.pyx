...
def code_parts_mag_aux(np.ndarray[ndim=3,dtype=UINT_t] X_edges,
                       np.ndarray[ndim=2,dtype=UINT_t] X_mags,
                       np.ndarray[ndim=2,dtype=UINT_t] X_aux,
                       np.ndarray[ndim=4,dtype=DTYPE_t] log_parts,
                       np.ndarray[ndim=4,dtype=DTYPE_t] log_invparts,
                       np.ndarray[ndim=2,dtype=DTYPE_t] log_mag_parts,
                       np.ndarray[ndim=2,dtype=DTYPE_t] log_mag_invparts,
                       np.ndarray[ndim=2,dtype=DTYPE_t] log_aux_parts,
                       np.ndarray[ndim=2,dtype=DTYPE_t] log_aux_invparts,
                       np.ndarray[ndim=2,dtype=UINT_t] X_thresh_mask):
    """
    At each location of `X`, find the log probabilities for each part and location. Outputs these part assignments in the same data dimensions as `X`. We also have auxiliary data containing information about the frequency channel and the magnitude of features in the frequency channel as well as wiener entropy, zero crossing rate, and the log power.  These are incorporated into the matching processing. Neighborhoods of `X` with edge counts lower than `threshold` are regarded as background and assigned zero.
    ...
    log_invparts : ndarray[ndim=4]
        Preprocessed inverse of `log_parts`, i.e. ``log(1-exp(log_parts))``.
    X_thresh_mask : ndarray[ndim=2,dtype=np.uint8]
        Has the same first two dimensinos as X, indicates which entries of
        X are going to be analyzed for patch fitting
