...
train_file_indices = np.load("data/train_file_indices.npy")
    ...
    for num_mix in num_mix_params:
        ...
        if num_mix == 1:
            ...
            if len(Es) > chunk_length:
                ...
                for i in xrange(len(Es)/chunk_length):
                    ...
                    if i == 0:
                        bem = bm.BernoulliMixture(num_mix,Es[start_idx:end_idx])
                        gc.collect()
                        bem.run_EM(.000001)
                    else:
...
#
# Want to diagnose what is going on with Es
#
...
first_pass_fnrs = np.array([5,7,10])
...
out.close()
detection_array = np.load('data/detection_array_%d.npy' % num_mix
                         )
detection_template_ids = np.load('data/detection_template_ids_%d.npy' % num_mix)
out = open('data/example_start_end_times.pkl','rb')
example_start_end_times = cPickle.load(out)
out.close()
outfile = np.load('data/linear_filter_%d.npy.npz'% num_mix)
linear_filters=tuple(outfile['arr_%d' % i] for i in xrange(len(outfile.files)))

outfile = np.load('data/c_%d.npy'%num_mix)


out = open('data/detection_lengths.pkl','rb')
detection_lengths = cPickle.load(out)
out.close()

detection_lengths = np.load('data/detection_lengths_%d.npy' % num_mix)
...
C0 = int(np.mean(tuple( t.shape[0] for t in templates))/3.+.5)
C1 = int( 33 * 1.5 + .5)
thresh_id = int(len(detection_clusters)* 7/100. + 5)

max_detect_vals = np.load('data/max_detect_vals_%d.npy'%num_mix)
fpr = np.load('data/fpr_%d.npy' % num_mix,
                )
...
num_mix=1
first_pass_fnrs = np.array([5,7,10])
out = open('data/detection_clusters_%d.npy' % num_mix,'rb')
detection_clusters = cPickle.load(out)
out.close()
# detection_array =
...
outfile = np.load('data/%d_templates.npz' % num_mix)
templates = tuple( outfile['arr_%d'%i] for i in xrange(len(outfile.files)))
detection_array = np.load('data/detection_array_%d.npy' % num_mix,detection_array)
(detection_array,
example_start_end_times,
      detection_lengths,
detection_template_ids)=gtrd.get_detection_scores_mixture_named_params(utterances_path,
                                                                       train_file_indices,
                                                                       detection_array,
                                                                       syllable,
                                                                       linear_filters_cs,S_config=sp,
...
int(len(detection_clusters)* fnr/100. + 5)
(pos_times,
 false_pos_times,
