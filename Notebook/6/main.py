...
#
"""
Experiment file here is meant to do an extaction of 'by' and then
from that build the different models and assess the detection performance
"""
import numpy as np

root_path = '/var/tmp/stoehr/Template-Speech-Recognition/'
data_path = root_path + 'Data/'
exp_path = root_path + 'Notebook/6/'
...
parts_clipped_bgd = np.clip(parts_avg_bgd.E,.1,.4)
np.save(tmp_data_path+'parts_clipped_bgd_102012.npy',parts_clipped_bgd)

out = open(tmp_data_path+'by_syllable_examples.pkl','wb')
cPickle.dump(syllable_examples,out)
out.close()

out = open(tmp_data_path+'by_parts_syllable_examples.pkl','wb')
cPickle.dump(parts_syllable_examples,out)
out.close()
